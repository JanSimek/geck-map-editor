...
public:
    ...
    // Move constructor and assignment work as expected
    ...
    /**
     * Calls all connected functions except one
     */
    void emit_all_but_one(int excludedConnectionID, Args... p) {
        for (auto const& it : _slots) {
...
private:
    mutable std::map<int, std::function<void(Args... args)>> _slots;
